...
public class AADD extends DD {
	...
	public static final double PRECISION = 1e-3d;
	...
	public int pruneNodes(int rid) {
		...
		PruneResult retp = pruneNodeDiff(getRNode(rid), 1d, true, PRUNE_PRECISION); 
	
		System.out.println("Used error: " + (PRUNE_PRECISION - retp._allow_err) + " / " + PRUNE_PRECISION); 
		return addRNodeRef(retp._rnode);
	}

	// Start at root
	public void pruneNodeDiff() {
		setRoot(pruneNodeDiff(_pRoot, 1d, true, PRUNE_PRECISION)._rnode);
	}
	...
	// error at the node.
	public PruneResult pruneNodeDiff(AADDRNode r, double range, boolean recurse, double allow_error) {
		...
		if ((ret._rnode = (AADDRNode) _hmPruneMap.get(_tmpADDRNode)) == null) {
			...
			// Recurse
			if (recurse) {
				PruneResult lowp  = pruneNodeDiff(low, range, true, ret._allow_err);
				low = lowp._rnode;
				PruneResult highp = pruneNodeDiff(high, range, true, ret._allow_err);
				high = highp._rnode;
			...
			if (/* check already linear! */(low2._nRefID != high2._nRefID)
				...
				case REPLACE_AVG: {
					...
					if (avg_size <= orig_size) {
						...
						ret = pruneNodeDiff(ret._rnode, range, true, ret._allow_err);
